...
void LogPutc(char c,void * arg);
   ...
   #endif
   #define _LOG(LogFlags,format, ...) if(!(LogFlags & LOG_DISABLED)) printf(format,## __VA_ARGS__)
...
// ALOG - Normal messages 
   ...
   // LOG - normal debug messages
   #define VLOG(format, ...) if(!(gLogFlags & LOG_VERBOSE_DISABLED)) _LOG(gLogFlags,"%s: " format,__FUNCTION__ ,## __VA_ARGS__)
   // LOG_R - raw debug messages (function name prefix not added) 
   #define VLOG_R(format, ...) if(!(gLogFlags & LOG_VERBOSE_DISABLED)) _LOG(gLogFlags,format,## __VA_ARGS__)
   #define VLOG_HEX(x,y) if(!(gLogFlags & LOG_VERBOSE_DISABLED)) LogHex(gLogFlags,x,y)
#else
   #define VLOG(format, ...)
